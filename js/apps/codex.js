/* ========================= Codex =========================
   NimbusOS's code editor. CodeMirror-powered syntax highlighting,
   a file-tree sidebar for jumping between files without leaving the
   window, Find/Replace (Ctrl/Cmd+F), and a Run button: .js files
   execute in a sandbox with console output captured below the editor;
   .html files render live in a preview pane.
...
const CodexApp = (() => {
  ...
  function fileIcon(name) {
    ...
    if (["js", "json", "css", "html", "htm", "jsx"].includes(ext)) return ICONS.code;
    if (["md", "txt", "markdown"].includes(ext)) return ICONS.editor;
  ...
  function open(path, opts) {
    ...
    root.innerHTML = `
      <div class="app-toolbar">
        <button class="app-btn" data-act="toggle-sidebar" title="Toggle files">${ICONS.explorer}</button>
        <button class="app-btn" data-act="new" title="New file">${ICONS.add}</button>
      ...
      <div class="ce-body">
        <div class="ce-sidebar" data-act="sidebar">
          <div class="ce-sidebar-head">Files</div>
          <div class="ce-tree" data-act="tree"></div>
        </div>
    ...
    const sidebar = root.querySelector('[data-act="sidebar"]');
    const tree = root.querySelector('[data-act="tree"]');
    ...
    function setEditorValue(text) {
      if (cm) { cm.setValue(text); cm.clearHistory(); }
    ...
    const winId = WM.createWindow({
      appId: "codex:" + (path || ("untitled-" + Date.now())),
      title: currentPath ? currentPath.split("/").pop() : "Untitled — Codex",
      icon: ICONS.code,
    ...
    function refreshTitle() {
      ...
      WM.setTitle(winId, (dirty ? "• " : "") + name + " — Codex");
    ...
    function renderTree() {
      tree.innerHTML = "";
      buildTreeLevel("/", tree, 0);
    }
    function buildTreeLevel(dirPath, container, depth) {
      const entries = FS.readDir(dirPath) || [];
      entries.forEach(node => {
        const nodePath = (dirPath === "/" ? "" : dirPath) + "/" + node.name;
        const row = document.createElement("div");
        row.className = "ce-tree-item" + (nodePath === currentPath ? " active" : "");
        row.style.paddingLeft = (8 + depth * 14) + "px";
        if (node.type === "dir") {
          const isOpen = expanded.has(nodePath);
          row.innerHTML = `<span class="ce-tree-caret">${isOpen ? "▾" : "▸"}</span><span class="ce-tree-icon">${ICONS.folder}</span><span class="ce-tree-label">${node.name}</span>`;
          row.addEventListener("click", () => {
            expanded.has(nodePath) ? expanded.delete(nodePath) : expanded.add(nodePath);
            renderTree();
          });
        } else {
          row.innerHTML = `<span class="ce-tree-caret"></span><span class="ce-tree-icon">${fileIcon(node.name)}</span><span class="ce-tree-label">${node.name}</span>`;
          row.addEventListener("click", () => loadFile(nodePath));
        }
        container.appendChild(row);
        if (node.type === "dir" && expanded.has(nodePath)) buildTreeLevel(nodePath, container, depth + 1);
      });
    }

    function confirmDiscardIfDirty(question) {
      if (!dirty) return true;
      return confirm(question);
    }

    function loadFile(newPath) {
      if (newPath === currentPath) { if (cm) cm.focus(); return; }
      if (!confirmDiscardIfDirty("You have unsaved changes. Discard them and open this file?")) return;
      const content = FS.readFile(newPath);
    ...
    function newFile() {
      if (!confirmDiscardIfDirty("You have unsaved changes. Discard them and start a new file?")) return;
      currentPath = null;
      dirty = false;
      setEditorValue("");
      setMode(null);
    ...
    root.querySelector('[data-act="toggle-sidebar"]').addEventListener("click", () => {
      sidebarOpen = !sidebarOpen;
      sidebar.style.display = sidebarOpen ? "flex" : "none";
      if (cm) setTimeout(() => cm.refresh(), 0);
    });
    root.querySelector('[data-act="new"]').addEventListener("click", newFile);
    ...
    function saveAs() {
      ...
      if (FS.writeFile(p, getValue())) {
        ...
        WM.setAppId(winId, "codex:" + currentPath);
    ...
    // Ctrl/Cmd+S save, Ctrl/Cmd+Enter run (Ctrl/Cmd+F find is handled by CodeMirror's search addon)
